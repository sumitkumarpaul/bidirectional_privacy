...
# Sumit:
- During compilation errors occurred:
	- All the required files for mbedtls include are not found
		- Solution: - Installed mbedtls using: sudo apt install libmbedtls-dev
			    - Then replace the content from https://github.com/Mbed-TLS/mbedtls/tree/08b04b11ff55a96f4021e5622b49e28a09417672/include to /user/include/

	- During compilation with the experimental code, problem occured like: sgx_report.h is not found.
	        - Solution: - Download the ubuntu package containing the header files from: https://download.01.org/intel-sgx/sgx_repo/ubuntu/pool/main/devel/libsgx-headers/libsgx-headers_2.13.103.1-focal1_amd64.deb
		            - Then installed the *.deb package using the command:sudo dpkg -i libsgx-headers_2.12.100.3-focal1_amd64.deb 
      - Install libxml2 headers by using: sudo apt install libxml2 libxml2-dev
