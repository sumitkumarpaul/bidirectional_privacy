...
static int example4(const char *filename, const xmlChar * xpathExpr,
                    const xmlChar * value);
//static int eval_xpath(const char* filename, const xmlChar* xpathExpr, const xmlChar* DTDPath);
static int eval_xpath(const char* filename, const xmlChar* xpathExpr);
...
#else
    if (eval_xpath(argv[1], BAD_CAST argv[2])) {
	return(-1);
    }
#endif
...
static int eval_xpath(const char* filename, const xmlChar* xpathExpr) {
...
#if 0
    if (xmlValidateDtd(NULL, doc, xmlParseDTD(NULL, DTDPath)) == 0) {
        fprintf(stderr,"Error: unable to evaluate with DTD\n");
        return(-1);
    }
#endif
    ...
    fprintf(stdout,"Number of the nodes: %d\n", xpathObj->nodesetval->nodeNr);
