...
Sample_walkthrough() {
  ...
  # Pre-install some data into the DU's enclave
  # did [0-> #DI:1][1-> #DI:10][2-> #DI:100][3-> #DI:1000][4-> #DI:1000]
  for i in "${!d_file[@]}"
  do
    echo "SaveData" >&3
  ...
  do
    ...
    echo -e "For ${d_file[$i]}, data processing time requirement is:\t $time_in_us us" >> $report_file
  done
...
num_itr=1 # This is a dummy variable
non_priv="${non_priv,,}"
log_timestamp="$(date +"%d_%m_%Y_%H-%M-%S")"
log_folder="Walkthrough_log_$log_timestamp/"
mkdir $log_folder
report_file="$log_folder/Overall_report.txt"

echo -e "============================================================================================================" >> $report_file
echo -e "Running through all the operations of the protocol" >> $report_file
